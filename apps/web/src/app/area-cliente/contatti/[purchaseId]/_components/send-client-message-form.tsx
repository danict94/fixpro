...
export function SendClientMessageForm({ purchaseId }: { purchaseId: string }) {
  const router          = useRouter()
  const [body, setBody] = useState('')
  const [error, setError] = useState<string | null>(null)

  const send = trpc.contacts.sendClientMessage.useMutation({
    onSuccess: () => {
      setBody('')
      setError(null)
      router.refresh()
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    if (!body.trim()) return
    send.mutate({ purchaseId, body: body.trim() })
  }
    ...
    <form onSubmit={handleSubmit} className="border-t pt-4 space-y-2">
      <div className="flex gap-2 items-end">
        <textarea
          ...
          placeholder="Scrivi all'impresa… (Invio per inviare, Shift+Invio per andare a capo)"
          rows={3}
