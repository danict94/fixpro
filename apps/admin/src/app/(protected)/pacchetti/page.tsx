...
export default async function PacchettiPage() {
  ...
  const packages = (await api.admin.packages.list()) as CreditPackage[]
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Gestisci i pacchetti acquistabili dalle imprese. Solo i pacchetti attivi sono visibili
            nel checkout.
          </p>
        </div>
      </div>
      ...
      {packages.length > 0 ? (
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {packages.map((pkg: CreditPackage) => (
                    <tr
                      key={pkg.id}
                      className={`border-b last:border-0 ${!pkg.active ? 'opacity-50' : ''}`}
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          {pkg.popular && <Badge className="text-xs">Popolare</Badge>}
                        </div>
                      </td>
        ...
        <div className="rounded-lg border border-dashed p-10 text-center">
          ...
          <p className="text-sm font-medium text-muted-foreground">
            Nessun pacchetto creato. Aggiungine uno per consentire agli imprenditori di acquistare
            crediti.
          </p>
        </div>
      )}
